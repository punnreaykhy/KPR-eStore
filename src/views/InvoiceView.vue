...
export default {
    ...
    computed: {
        ...
        generateRandomOrderId() {
            ...
            // Combine the timestamp and random number to create the orderID
            ...
            <div class="d-flex">
                <div class="w-50">
                    <div class="d-flex gap-4 align-items-center">
                        <div id="thick"></div>
                        <div>
                            <div>Order #{{generateRandomOrderId}}</div>
                            <div class="fs-4 fw-bold">Thank You, {{ customerInfo.firstName }}!</div>
                        </div>
                    </div>
                    <hr>
                    <br>
                    <div>Your order is confirmed. <br>We’ve accepted your order and we’re getting it ready.</div>
                    <hr>
                    <h2>Customer Information:</h2>
                    <div class="py-3">
                        <div class="fw-bold">Shipping Address</div>{{ customerInfo.house }} {{ customerInfo.street }},
                ...
                <div class="w-50 d-flex justify-content-center">
                    <div class="summary p-4 d-flex flex-column gap-4 rounded-4 border border-4"
                        ...
                        <div class="overflow-auto" style="height: 14em">
                            <div v-for="p in products" :key="p.id" class="d-flex align-items-center border border-1">
                                <div class="w-75 d-flex flex-column justify-content-between pt-2 px-2">
                                    <h5 class="">{{ p.name }}</h5>
                        ...
                        <div class="d-flex mt-5 justify-content-between">
                            <h5>Total</h5>
                            <div class="d-flex">
                                <h5>{{ Number(this.$store.state.totalPrice) + Number((this.$store.state.totalPrice *
                                    this.$store.state.tax).toFixed(2)) }}</h5>
                                $
                            </div>
                        </div>
